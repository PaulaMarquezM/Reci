...
| Pieza | Estado | Dónde |
|---|---|---|
| Chasis + 4 motorreductores TT + 2×L298N | Comprado, guía de ensamble lista | `docs/product/CONEXIONES.md` ETAPA 3 |
| 2× sensor ultrasónico HC-SR04 | Comprado, guía de ensamble lista | `docs/product/CONEXIONES.md` ETAPA 5 (con PIR) |
| `robot_points` (tabla con `id`, `name`, `lat`, `lng`) | Ya existe en Supabase | `web/supabase/migrations/20260602000001_schema_v1.sql` |
| `robot_positions`, `POST /api/robot/position` | Ya existe — el robot puede reportar dónde está | `docs/product/API-ROBOT.md` §3 |
| Despacho de llamadas por polling (`GET /api/robot/calls/next`) | Ya implementado | `docs/product/PLAN.md` — decisiones técnicas |
...
- Es literalmente lo que ya menciona `docs/product/PLAN.md` en la Fase 2 ("prueba de
...
```text
Usuario pide "ven al punto X" (ya implementado, ver docs/product/API-ROBOT.md §1-2)
        ↓
