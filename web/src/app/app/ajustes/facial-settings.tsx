...
export function FacialSettings({
  ...
  return (
    <section className="space-y-5">
      ...
      ) : <button type="button" onClick={() => setShowConsent(true)} disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-[15px] py-3.5 text-[15px] font-bold text-white disabled:opacity-50" style={{ background: 'var(--glass)' }}><Icon name="camera" size={19} stroke="#fff" />{busy ? 'Registrando…' : 'Elegir 3–5 fotos'}</button>}
      {error ? <p className="rounded-xl px-3 py-2 text-[13px] font-medium" style={{ background: 'oklch(0.95 0.04 25)', color: 'var(--flame)' }}>{error}</p> : null}
    </section>
  )
}
